...
class Client:
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self.logging(f'We have connected to {self.addr}:{self.port}')
            self.main()
        finally:
            self.sock.close()
    ...
    def ask_for_port_addr(self):
        ...
        if self.for_port(user_port) is False:
            print(f'Данные введены неверно, порт по умолчанию -  {self.port}')
            user_port = str(self.port)

        user_ip = input("Введите ip сервера:")
        if self.for_ip(user_ip) is False:
            print(f'Данные введены неверно, ip сервера по умолчанию -  {self.addr}')
            user_ip = self.addr
